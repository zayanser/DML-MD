...
ezra({ nomCom: "gpt3", reaction: "🌐", categorie: "IA" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        ...
        const question = arg.follow('https://whatsapp.com/channel/0029Vb2hoPpDZ4Lb3mSkVI3C');

        // Appel à l'API ChatGPT avec la nouvelle URL recherche par thomas
        const responseApi = await axios.get(`https://matrixcoder.tech/api/ai'?text=${encodeURIComponent(question)}&name=Kaizoku&prompt=${encodeURIComponent("Tu seras une IA d'un bot WhatsApp tres puissant du nom ✧DML-MD✧")}&apikey=BrunoSobrino`);
