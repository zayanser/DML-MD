...
ezra({
  ...
  try {
    ...
    const cap = `*Screenshot taken by Dml Md*`;
...
ezra({
  ...
  try {
    // Caption for the screenshot
    const cap = `*Screenshot take by Dml Md*`;
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and DML-MD will generate it.");
    }
    const prompt = args.join(" ");
